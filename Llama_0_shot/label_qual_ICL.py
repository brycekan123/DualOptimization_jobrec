...
for job_id, group in qual_train_df.groupby('item'):
    ...
    n_neg = (group['label_qual'] == 0).sum()
    if n_pos >= 1 and n_neg >= 1:
        example_jobs.append(job_id)
    if len(example_jobs) >= 2:
        break

if len(example_jobs) < 2:
    print("❌ ERROR: Need at least 2 jobs with both positive and negative examples in training data!")
    exit(1)
...
for job_id in example_jobs:
    ...
    # Get 1 positive
    pos_sample = job_data[job_data['label_qual'] == 1].sample(n=1, random_state=42).iloc[0]
    icl_examples.append({
        'job_id': job_id,
...
def build_icl_prompt(user_id, item_id, examples):
    ...
    for i, ex in enumerate(examples, 1):
        ...
        ex_item_features = item_dict.get(ex['job_id'], {})
        ...
        examples_text += f"Item ID: {ex['job_id']}\n"
        examples_text += f"Item Features:\n{ex_item_info}\n\n"
        examples_text += f"Rating: {ex['score']}\n"
        examples_text += f"(This user was {'selected' if ex['label'] == 1 else 'not selected'} for this job)\n\n"
...
for run_idx in range(NUM_RUNS):
    ...
    # Show rank distribution to understand variance
    if scores_data:
...
averaged_csv = os.path.join(OUTPUT_DIR, "baseline_averaged_metrics_qual_icl.csv")
...
individual_csv = os.path.join(OUTPUT_DIR, "baseline_individual_runs_qual_icl.csv")
...
if all_runs_scores_data[0]:
    ...
    scores_csv = os.path.join(OUTPUT_DIR, "baseline_scores_qual_icl_run1.csv")
...
print("=" * 100)
print(f"\n💡 Summary:")
print(f"  - Tested on {len(qual_test_batches)} batches from qual_test.csv (QUALIFICATION DATA)")
print(f"  - Used {NUM_ICL_EXAMPLES} in-context examples in every prompt")
print(f"  - ICL examples from jobs: {example_jobs}")
